...
namespace CV_Creator.templates
{
    public partial class Template3 : Form
    {
                                                    ...
                                                    if (checkContent(certificates))
                                                    {
                                                        if (checkContent(work))
                                                        {
                                                            if (checkContent(hobbies))
                                                            {
                                                                ...
                                                                {
                                                                    if (checkContent(skills))
                                                                    {
                                                                        if (checkContent(about))
                                                                        {
                                                                            createCV();
                                                                        }
                                                                        else
                                                                        {
                                                                            MessageBox.Show("About You Field cannot be empty", "Error",
                            MessageBoxButtons.OK, MessageBoxIcon.Error);
                                                                        }
                                                                    }
                                                                    else
                                                                    {
                                                                        MessageBox.Show("Skills Field cannot be empty", "Error",
                        MessageBoxButtons.OK, MessageBoxIcon.Error);
                                                                    }
                                                                }
                                                                else
        ...
        {
            Helper.copyTemplate(fullname.Text.ToLower(), phone.Text.ToLower(), Properties.Resources.template3);
            try
            {
                ...
                if (File.Exists((string)filename))
                {
                    ...
                    this.FindAndReplace(wordApp, aDoc, "Skills", skills.Text);
                    this.FindAndReplace(wordApp, aDoc, "About", about.Text);
