...
namespace CV_Creator.templates
{
    public partial class Custom: Form
    {
        ...
        {
            InitializeComponent();
            this.path = Helper.IMPORT_DOC+path;
        ...
        private string createDocument(string path)
        {
            ...
            if (!Helper.fileExists(Helper.FULL_WORKSPACE + fullname.Name))
            {
                f.CopyTo(Helper.FULL_WORKSPACE + fullname.Text.ToLower()+".docx");
            }
            else
            {
                MessageBox.Show("File selected in one of the field already exists\n" + fullname.Text.ToLower() + ".docx", "Error",
        MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            return f.Name;
        }
